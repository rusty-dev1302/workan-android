...
export class ListingService {
  ...
  getListingsByFilters(subcategory: string, geoHash: string, sortBy: string, averageDistance: number, pageNumber: number): Observable<Listing[]> {
    const getUrl = `${this.baseUrl}/search?subcategory=${subcategory}&&geoHash=${geoHash}&&sortBy=${sortBy}&&averageDistance=${averageDistance}&&pageNumber=${pageNumber}`;
    return this.httpClient.get<Listing[]>(getUrl)
  }
